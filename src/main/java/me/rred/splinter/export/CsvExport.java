...
public class CsvExport {
    ...
    public static void export(List<SplinterSet> sets, Path out) throws IOException {
        String headers = "Set,Time"; // for now just hardcode this since there's only one type of file to export
        ...
        try (BufferedWriter bw = Files.newBufferedWriter(out, StandardOpenOption.CREATE, StandardOpenOption.TRUNCATE_EXISTING)){
            ...
            for (SplinterSet set : sets) {
                ...
                String nameEntry = set.getName().trim();

                for (Long time : set.getTimes()) {
                    String timeEntry = TimerFormatter.formatSeconds(time);
